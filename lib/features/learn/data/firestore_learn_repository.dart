...
class FirestoreLearnRepository implements LearnRepository {
  ...
  CollectionReference<Map<String, dynamic>> _progress(String userId) =>
      _db.collection('student_profiles').doc(userId).collection('lessonProgress');
  ...
  Future<List<LearnSubject>> fetchSubjects({
    ...
    final subjectsSnap = await _subjects(classLevel)
    ...
    for (final subjectDoc in subjectsSnap.docs) {
      ...
      final allowedSeries = List<String>.from(sd['allowedSeries'] as List? ?? []);
      ...
      // Fetch chapters
      final chaptersSnap =
          await _chapters(classLevel, subjectId).orderBy('order').get();
      ...
      for (final chapterDoc in chaptersSnap.docs) {
        ...
        for (final entry in progressMap.entries) {
          if (entry.key.startsWith(prefix)) {
            totalProgress +=
                (entry.value['progress'] as num?)?.toDouble() ?? 0;
          }
        }
        final completion =
            lessonsCount > 0 ? (totalProgress / lessonsCount).clamp(0.0, 1.0) : 0.0;

        chapterSummaries.add(LearnChapterSummary(
          id: chapterId,
  ...
  Future<LearnSubjectDetail> fetchSubjectDetail({
    ...
    final progressSnap = await _progress(userId)
        .where(FieldPath.documentId,
            isGreaterThanOrEqualTo: '${subjectId}_')
    ...
    for (final chapterDoc in chaptersSnap.docs) {
      ...
      final lessonsSnap = await _lessons(classLevel, subjectId, chapterId)
          .where('status', isEqualTo: 'published')
          .orderBy('order')
          .get();
  ...
  Future<LearnChapter> fetchChapter({
    ...
    final chapterDoc =
        await _chapters(classLevel, subjectId).doc(chapterId).get();
    final cd = chapterDoc.data();
  ...
  Future<LearnLesson> fetchLesson({
    ...
    final lessonDoc =
        await _lessons(classLevel, subjectId, chapterId).doc(lessonId).get();
