...
| --- | --- | --- | --- | --- |
| Quiz attempts | `lib/features/quiz/data/firestore_quiz_attempt_service.dart` | Previously created `quiz_attempts/{autoId}` from client-computed answers, score, corrections, and legacy `xpAwarded`. | A client could forge score, points, answer details, quiz id, and student id. | Cloud Function `submitQuizAttempt`, with canonical `pointsAwarded`. |
| User points | `lib/features/quiz/data/firestore_quiz_attempt_service.dart` | Previously merged legacy `users/{uid}.xp += attempt.xpAwarded`. | A client controlled the increment amount and could inflate points. | Cloud Function transaction writing `points`. |
| Student profile points | `lib/features/quiz/data/firestore_quiz_attempt_service.dart` | Previously merged legacy `student_profiles/{uid}.xp += attempt.xpAwarded`. | A client controlled academic progression fields. | Cloud Function transaction writing `points`. |
...
| Avatar URL | `lib/features/profile/services/profile_image_service.dart` | Uploads `avatars/{uid}/profile.jpg` and updates `users/{uid}.photoUrl`. | Storage lacks MIME/size constraints; Firestore update must not allow role/points edits. | Client upload with strict Storage rules plus user update whitelist. |
| Student registration | `lib/features/student_registration/data/firebase_student_registration_repository.dart` | Creates `users/{uid}` and `student_profiles/{uid}` with role `student`, academic context, and initial points/level. | Initial profile creation includes sensitive fields; allowed only at zero-value bootstrap. | Client create for student bootstrap only, then server-owned academic fields. |
...
- `student_profiles/{uid}` previously allowed owner updates to legacy `xp`, `level`, `streak`, and other academic fields.
...
- `users/{uid}` previously allowed owner updates to `role`, legacy `xp`, and establishment fields.
...
- `student_profiles/{uid}`: non-authoritative preferences and consents only. Initial creation may set `points = 0`, `level = 1`, and empty/default streak values, but later client updates cannot change academic metrics. Legacy clients may still bootstrap `xp = 0`; subsequent writes are canonical.
