...
void main() {
  group('Phase E: Audit & Security Contracts (account_management_audit)', () {
    test('StudioAuditEvent models an immutable audit entry with required motif', () {
  ...
  group('Phase E: Full 31 Modules Navigation Completeness', () {
    test('All 31 canonical StudioModule entries are defined and partitioned into sections', () {
      expect(StudioModule.values.length, 31);

      // Verify sections distribution
      final orgModules = StudioModule.values.where((m) => m.section == StudioSection.organization).toList();
      final contentModules = StudioModule.values.where((m) => m.section == StudioSection.content).toList();
      final financeModules = StudioModule.values.where((m) => m.section == StudioSection.finance).toList();
      final operationsModules = StudioModule.values.where((m) => m.section == StudioSection.operations).toList();
      final systemModules = StudioModule.values.where((m) => m.section == StudioSection.system).toList();
