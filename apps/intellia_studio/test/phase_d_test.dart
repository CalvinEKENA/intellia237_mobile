...
void main() {
  group('Phase D: Study Reserve Contracts & Policies', () {
    test('StudioStudyReserve computes consumption ratio and critical status', () {
  ...
  group('Phase D: Mobile Money & Financial Auditing', () {
    test('StudioPaymentRequest accurately formats XAF currency and tracks review metadata', () {
    ...
    test('LegacyFinancialRecord preserves collection classification and read-only integrity', () {
      ...
      expect(legacyCredit.classification, DataCollectionClassification.legacyRequiresClassification);
      expect(legacyCredit.rawData['balance'], 200);
    });

    test('StudioSubscriptionPlan tracks Firestore provisioning status vs fallback', () {
