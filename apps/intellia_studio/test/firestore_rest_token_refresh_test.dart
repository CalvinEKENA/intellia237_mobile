...
void main() {
  group('FirestoreRestClient Token Refresh and Resilience', () {
    test(
      'Normal authenticated request passes without triggering refresh',
      () async {
        ...
        expect(
          refreshCalls,
          0,
          reason: 'No refresh call should be made on 200 OK',
        );
    ...
    test(
      '401 response triggers single refresh call and retries original request successfully',
      () async {
          ...
          httpClient: MockHttpClient((req) async {
            ...
            if (req.headers['authorization'] == 'Bearer expired_token') {
              return http.Response(
                ...
                'Bearer fresh_token_999') {
              return http.Response(
                jsonEncode({
                  'documents': [
                    {
                      'name':
                          'projects/p/databases/(default)/documents/establishments/est_1',
                      'fields': {
                        'name': {'stringValue': 'Lycée Général Leclerc'},
                      },
                    },
                  ],
                }),
    ...
    test(
      'Concurrent 401 responses share single in-flight refresh (no refresh storm)',
      () async {
          ...
          httpClient: MockHttpClient((req) async {
            if (req.headers['authorization'] == 'Bearer expired_storm_token') {
              return http.Response('{"error":"UNAUTHENTICATED"}', 401);
                ...
                'Bearer consolidated_fresh_token') {
              return http.Response(
                jsonEncode({'documents': []}),
                200,
                headers: {'content-type': 'application/json'},
        ...
        expect(
          refreshCalls,
          1,
          reason: 'Refresh should only be called ONCE for concurrent 401s',
        );

        // Complete the single refresh
        currentSession = currentSession.copyWith(
          idToken: 'consolidated_fresh_token',
        );
        ...
        expect(
          refreshCalls,
          1,
          reason: 'Still exactly 1 refresh call performed',
        );
      },
    );
    ...
    test('Second 401 after refresh triggers session expiration', () async {
      ...
      final client = FirestoreRestClient(
        ...
        tokenRefresher: () async {
          refreshCalls++;
          currentSession = currentSession.copyWith(
            idToken: 'refreshed_but_still_invalid',
          );
          return currentSession;
        },
        onSessionExpired: () async {
      ...
      expect(
        expiredCalls,
        1,
        reason: 'Persistent 401 must trigger session expiration',
      );
    });

    test(
      'Refresh failure surfaces error and triggers session expiration',
      () async {
        var currentSession = _createSession(idToken: 'expired_token');
        int expiredCalls = 0;
        ...
        expect(
          expiredCalls,
          1,
          reason: 'Null refreshed session must trigger expiration',
        );
      },
    );
  });
