...
class _LessonEditorScreenState extends ConsumerState<LessonEditorScreen> {
  ...
  List<StudioContentBlock> blocks = [
    const StudioContentBlock(
      ...
      body: 'Dans cette leçon, nous aborderons la notion fondamentale de continuité sur un intervalle.',
    ),
    const StudioContentBlock(
      id: 'blk_02',
      type: ContentBlockType.callout,
      title: 'Théorème clé (TVI)',
      body: 'Si f est continue sur [a, b] et k compris entre f(a) et f(b), alors il existe c in [a, b] tel que f(c) = k.',
    ),
    const StudioContentBlock(
      id: 'blk_03',
      type: ContentBlockType.text,
      title: 'Exemple d’application guidée',
      body: 'Montrons que l’équation x^3 + x - 1 = 0 admet au moins une solution réelle dans ]0, 1[.',
    ),
  ];
  ...
  void initState() {
    super.initState();
    titleCtrl = TextEditingController(text: 'Théorème des Valeurs Intermédiaires (TVI)');
    summaryCtrl = TextEditingController(text: 'Énoncé, interprétation graphique et méthode de dichotomie.');
                ...
                OutlinedButton.icon(
                  onPressed: () {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Brouillon enregistré en local.')),
                    );
                  },
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            TextField(
                              ...
                              decoration: const InputDecoration(labelText: 'Titre officiel'),
                            ),
                            const SizedBox(height: 16),
                            TextField(
                              controller: summaryCtrl,
                              maxLines: 3,
                              decoration: const InputDecoration(labelText: 'Résumé pédagogique'),
                            ),
                            const SizedBox(height: 16),
                            TextField(
                            ...
                            Container(
                              ...
                              child: const Row(
                                children: [
                                  Icon(Icons.security_rounded, size: 20, color: StudioColors.navyPrimary),
                                  SizedBox(width: 8),
                                  Expanded(
                                    child: Text(
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    OutlinedButton.icon(
                                      onPressed: () => _addBlock(ContentBlockType.text),
                                      icon: const Icon(Icons.text_fields_rounded, size: 16),
                                      label: const Text('Texte'),
                                    ),
                                    const SizedBox(width: 8),
                                    OutlinedButton.icon(
                                      onPressed: () => _addBlock(ContentBlockType.callout),
                                      icon: const Icon(Icons.info_outline_rounded, size: 16),
                                      label: const Text('Encadré'),
                                    ),
                                    const SizedBox(width: 8),
                                    OutlinedButton.icon(
                                      onPressed: () => _addBlock(ContentBlockType.video),
                                          ...
                                          Row(
                                            ...
                                            children: [
                                              Row(
                                                children: [
                                                  Icon(
                                                    blk.type == ContentBlockType.callout
    ...
    setState(() {
      blocks.add(
        StudioContentBlock(
          ...
          type: type,
          title: type == ContentBlockType.callout ? 'Formule importante' : 'Nouveau paragraphe',
