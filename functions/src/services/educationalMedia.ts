...
export function parseAssetPath(path: unknown) {
  if (typeof path !== "string") throw new HttpsError("invalid-argument", "Chemin média requis.");
  const p = path.split("/");
  if (p.length !== 7 || p[0] !== "educational_assets" || p.slice(1).some(s => !s.trim() || s === "." || s === ".." || /[\\\x00-\x1f]/.test(s))) {
    throw new HttpsError("invalid-argument", "Chemin média invalide.");
  }
...
export async function validateLessonMedia(lesson: DocumentData, store: Bucket = bucket) {
  ...
  for (const block of lesson.contentBlocks || []) {
    ...
    if (!["image", "audio", "video", "pdf"].includes(block.mediaType)) throw new HttpsError("invalid-argument", "Type de média inconnu.");
    ...
    if (asset.scopeId !== scope || asset.classLevel !== lesson.classLevel || asset.subjectId !== lesson.subjectId || asset.lessonId !== lesson.id) {
      throw new HttpsError("permission-denied", "Le média ne correspond pas à cette leçon et à son périmètre.");
    }
    const file = store.file(asset.path);
    let metadata;
    try { [metadata] = await file.getMetadata(); }
    catch { throw new HttpsError("failed-precondition", "Fichier absent ou téléversement inachevé. Réimportez le média."); }
    ...
    if (!Number.isFinite(size) || size <= 0 || size > limits[block.mediaType] * 1024 * 1024 || !types[block.mediaType].includes(metadata.contentType || "") || (block.mimeType && block.mimeType !== metadata.contentType)) {
      throw new HttpsError("failed-precondition", "Format ou taille du média non conforme.");
    }
    let durationSeconds = block.durationSeconds;
    if (block.mediaType === "video") {
      if (block.mimeType !== "video/mp4" || !asset.fileName.toLowerCase().endsWith(".mp4")) throw new HttpsError("failed-precondition", "Une vidéo MP4 est requise.");
      try {
      ...
      } catch { throw new HttpsError("failed-precondition", "Vidéo illisible : exportez un MP4 H.264 avec audio AAC puis réimportez-le."); }
    }
...
export function createEducationalMediaHandler(firestore: Firestore = db, store: Bucket = bucket) {
  return async (request: CallableRequest) => {
    ...
    if (entry && entry.path !== asset.path) throw new HttpsError("permission-denied", "Référence média incompatible.");
    if (request.data?.action === "delete") {
      if (!staff) throw new HttpsError("permission-denied", "Suppression non autorisée.");
      await firestore.runTransaction(async tx => {
        const current = (await tx.get(ledger)).data();
        if (current?.path && current.path !== asset.path) throw new HttpsError("permission-denied", "Référence média incompatible.");
        if (current?.lessonPath) {
          const lesson = (await tx.get(firestore.doc(current.lessonPath))).data();
          if (lesson?.contentBlocks?.some((b: DocumentData) => b.storagePath === asset.path)) throw new HttpsError("failed-precondition", "Enregistrez le retrait du média de la leçon avant de supprimer le fichier.");
        }
    ...
    if (entry && entry.state !== "ready") throw new HttpsError("not-found", "Média indisponible.");
    if (!staff && (!entry?.lessonPath || !await publishedLessonAllows(firestore, entry.lessonPath, actor, profile))) throw new HttpsError("permission-denied", "Ce média ne fait pas partie de votre catalogue publié.");
    if (!staff) {
      const lesson = (await firestore.doc(entry!.lessonPath).get()).data();
      if (!lesson?.contentBlocks?.some((b: DocumentData) => b.storagePath === asset.path)) throw new HttpsError("not-found", "Média retiré de cette leçon.");
    }
    try {
    ...
    } catch { throw new HttpsError("unavailable", "Le média est indisponible. Réessayez dans quelques instants."); }
  };
}
